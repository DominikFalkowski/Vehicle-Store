...
//                    case 2 -> runVehicleStoreMenu();
    ...
    private void runChooseMenu() {
        int option = manufacturerMenu();
        while (option != 0) {
            switch (option) {
                case 1 -> vehicleReportsMenu();
                case 2 -> manufacturerReportsMenu();
                default->  System.out.println("Invalid option entered" + option);
            }
            ScannerInput.readNextLine("\n Press the enter key to continue");
            option = manufacturerMenu();
...
//    todo write listAllVehicleByChosenManufacturer()
